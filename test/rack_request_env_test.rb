...
class RackRequestEnvTest < Test::Unit::TestCase
  ...
  def test_report_name_should_include_http_host
    ...
    re = RequestEnv.new({'PATH_INFO' => '/path2',
                          'HTTP_HOST' => 'localhost'},
                        start_time)
    assert_equal('localhost/path2', re.to_report[:name])
  end
  ...
  def test_report_name_should_include_request_method
    ...
    re = RequestEnv.new({'REQUEST_METHOD' => 'GET',
                          ...
                          'HTTP_HOST' => 'localhost'},
                        start_time)
    assert_equal('GET localhost/path2', re.to_report[:name])
  end
  ...
  def test_to_report
    ...
    re = RequestEnv.new({'PATH_INFO' => '/path2',
                        ...
                        start_time)
    rep = re.to_report
    assert_equal 'localhost/path2?hello=world', rep[:name]
    assert_equal start_time.iso8601, rep[:start_at]
    assert rep[:end_at]
  end
